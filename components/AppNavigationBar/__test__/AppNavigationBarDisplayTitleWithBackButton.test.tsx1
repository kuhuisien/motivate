...
jest.mock("next/router", () => ({
  ...
  useRouter: () => ({
    pathname: "/habit/create", //module factory of `jest.mock()` is not allowed to reference any out-of-scope variables
    back: jest.fn(),
  }),
}));
...
describe("AppNavigationBar test displaying NavigationBar when it is feature page", () => {
  ...
  it("should render back button when it is configured to do so", () => {
    expect(wrapper.find(Button).length).toBe(1);
  });

  it("should have click handler for back button", () => {
